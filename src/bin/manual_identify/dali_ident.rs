...
impl DaliConfigurationDriver {
	...
	DaliConfigurationDriver{hw_driver, low_level: MASK, high_level: MASK}
    }
    /*
...
impl ConfigurationDriver for DaliConfigurationDriver {
	...
	let hw_driver = self.hw_driver.clone();
        Box::pin(async move {
            ...
            for g in gears.iter() {
                swaps.push((Short::new((Into::<u16>::into(g.id.clone()) -1) as u8), Short::new((Into::<u16>::into(g.conf.clone()) -1) as u8)));
            }
            let mut cmd = Commands102::from_driver(driver, PRIORITY_1);
