...
use std::fmt;
use std::error::Error;
use crate::drivers::simulator::device::{DaliSimHost,
					DaliSimEvent,
					DaliSimDevice};
...
impl DaliSimDevice for DaliSimDriverDevice
{
	...
	if let Ok(frame) = DaliFrame::try_from(&event.event_type) {
	    ctxt.last_transition = 
		event.timestamp + timing::frame_duration(&frame);
	}
	...
	if let Some(pending_result) = ctxt.pending_result.take() {
		...
		pending_result.reply.send(
		    if pending_result.expect_answer {
	...
	if event.source_id == ctxt.source_id {return None}
	let mut sent_answer = false;
	match event {
	    DaliSimEvent{event_type: DaliBusEventType::Frame8(answer),
			     ..} => {
		if let Some(pending_result) = ctxt.pending_result.take() {
		    pending_result.reply.send(
			DaliSendResult::Answer(*answer))
			.unwrap_or(());
		    sent_answer = true;
		}
	    },
	    DaliSimEvent{event_type: DaliBusEventType::FramingError, ..} => {
		if let Some(pending_result) = ctxt.pending_result.take() {
		    pending_result.reply.send(
			DaliSendResult::Framing).unwrap_or(());
		    sent_answer = true;
		}
	    },
	    _ => {}
	...
	if let Some(pending_result) = ctxt.pending_result.take() {
	    pending_result.reply.send( DaliSendResult::Timeout)
		.unwrap_or(());
	}
	...
	if !sent_answer {
	    let DaliSimEvent{timestamp, event_type, ..} = event;
	    let bus_event = DaliBusEvent{timestamp: *timestamp,
...
impl DaliDriver for DaliSimDriver
{
	...
	if let Ok(ref mut ctxt) = &mut self.ctxt.lock() {
		...
		Some({
		    ...
		    ev.timestamp = sim_event.timestamp 
			+ frame_dur
	    ...
	    return Box::pin(future::ready(DaliSendResult::DriverError("Context lock failed".into())))
	}
	...
	Box::pin(async move {
		...
		return 	DaliSendResult::DriverError(
		    "Sending to queue failed".into())
	    }
	    if let Some(sim_event) = sim_event2 {
		tokio::time::sleep_until(tokio::time::Instant::from_std(
		    sim_event.timestamp)).await;
		if host.send_event(sim_event).await.is_err() {
		    return DaliSendResult::DriverError(
			"Sending to queue failed".into())
		}
		...
		match tokio::time::timeout(Duration::from_millis(50),
					   answer_recv).await {
		    Ok(Ok(res)) => res,
		    Ok(Err(_)) => DaliSendResult::DriverError(
			"No answer was queued".into()),
