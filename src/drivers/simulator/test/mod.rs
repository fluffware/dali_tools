...
async fn add_sim_device()
{
    ...
    match driver.send_frame16(&[cmd::INITIALISE,0x00],
                              Flags::SendTwice(true)).await {
	DaliSendResult::OK => {},
    ...
    match driver.send_frame16(&[cmd::COMPARE,0x00], Flags::ExpectAnswer(true)).await
    {
    ...
    match driver.send_frame16(&[cmd::COMPARE,0x00], 
			      Flags::ExpectAnswer(true)).await
    {
    ...
    let res = driver.send_frame16(&[cmd::TERMINATE,00], Flags::Empty).await;
    println!("Sent: {:?}", res);
...
async fn discover()
{
    ...
    let sim = Arc::new(Mutex::new(Box::new(driver) as Box<dyn dali::drivers::driver::DaliDriver>));
...
async fn test_queries()
{
    ...
    assert_eq!(driver.send_frame16(&[3<<1, cmd::QUERY_STATUS], 
                                   Flags::ExpectAnswer(true)).await.check_answer().unwrap(),
               0u8);
    ...
    assert_eq!(driver.send_frame16(&[4<<1, cmd::QUERY_STATUS], 
                                   Flags::ExpectAnswer(true)).await.check_answer().unwrap(),
               0x3cu8);

    match driver.send_frame16(&[4<<1, cmd::QUERY_CONTROL_GEAR_PRESENT],
                           Flags::ExpectAnswer(true)).await {
        DaliSendResult::Answer(0xff) => {},
        r => panic!("Invalid answer: {:?}", r)
    }
    
    match driver.send_frame16(&[5<<1, cmd::QUERY_CONTROL_GEAR_PRESENT],
                           Flags::ExpectAnswer(true)).await {
