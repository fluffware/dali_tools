use super::driver::{DaliBusEventResult, DaliFrame, DaliSendResult};
...
pub fn send_frame(
    req_tx: &mut mpsc::Sender<DALIreq>,
    cmd: &DaliFrame,
    flags: Flags,
    ...
    match req_tx.try_send(req) {
        ...
        Err(_) => Box::pin(std::future::ready(DaliSendResult::DriverError(
            "Failed to queue command".into(),
        ))),
    }
}
pub fn next_bus_event(
    monitor_tx: &mut mpsc::Sender<oneshot::Sender<DaliBusEventResult>>,
