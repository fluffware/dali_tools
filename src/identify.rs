...
use dali::drivers::send_flags::{EXPECT_ANSWER, SEND_TWICE,
				PRIORITY_1, PRIORITY_5,
				NO_FLAG};
#[macro_use]
extern crate clap;
...
async fn identify(driver: &mut dyn DaliDriver, space: u8, mark: u8) 
		  -> Result<(), Box<dyn Error>>
{
    send_set_dtr0(driver, 0, PRIORITY_5).await.check_send()?;
    send_device_cmd(driver, &Broadcast, cmd::SET_FADE_TIME,
                    SEND_TWICE|PRIORITY_1).await.check_send()?;
    send_device_cmd(driver, &Broadcast, cmd::SET_EXTENDED_FADE_TIME,
                    SEND_TWICE|PRIORITY_1).await.check_send()?;
    let mut last = std::time::Instant::now();
    send_device_level(driver, &Broadcast, mark,NO_FLAG).await.check_send()?;
    sleep_delta(&mut last, BIT_TIME);
    send_device_level(driver, &Broadcast, space,NO_FLAG).await.check_send()?;
    ...
    for b in 0..6 {
	    ...
	    .await.check_send()?;
        sleep_delta(&mut last, HALF_BIT_TIME);
        send_device_level(driver, &Broadcast, next,NO_FLAG).await.check_send()?;
...
async fn identify_setup(driver: &mut dyn DaliDriver)
			-> Result<(), Box<dyn Error>>
{
    send_device_cmd(driver, &Broadcast,
                    cmd::RECALL_MIN_LEVEL,NO_FLAG).await.check_send()?;
    for i in 0..64 {
        ...
        send_device_cmd(driver, &Short::new(i+1),
                        cmd::RECALL_MAX_LEVEL,NO_FLAG).await.check_send()?;
        for b in 0..6 {
            ...
            send_device_cmd(driver, &Short::new(i+1),
                            cmd,SEND_TWICE).await.check_send()?;
        }
        send_device_cmd(driver, &Short::new(i+1),
                        cmd::RECALL_MIN_LEVEL,NO_FLAG).await.check_send()?;
    }
